        ...
        for (NSDictionary* collectionType in collectionTypes) {
            ...
            [[PHAssetCollection fetchAssetCollectionsWithType:[collectionType[@"type"] integerValue] subtype:PHAssetCollectionSubtypeAny options:nil] enumerateObjectsUsingBlock:^(PHAssetCollection* collection, NSUInteger idx, BOOL* stop) {
                ...
                if (collection != nil && collection.localizedTitle != nil && collection.localIdentifier != nil &&
                    ...
                    PHAsset *firstAsset = [result lastObject];
                    NSLog(@"%@",firstAsset);
                    ...
                    if (result.count > 0) {
                        ...
                        if ([collection.localizedTitle isEqualToString:@"Camera Roll"] && collection.assetCollectionType == PHAssetCollectionTypeSmartAlbum) {
                            cameraRoll = @{
                                ...
                                @"firstAsset": firstAsset.localIdentifier,
            ...
            if(isVideo){
                ...
                [[PHImageManager defaultManager] requestAVAssetForVideo:assets[0] options:nil resultHandler:^(AVAsset *asset, AVAudioMix *audioMix, NSDictionary *info) {
                    ...
                    if ([asset isKindOfClass:[AVURLAsset class]]) {
                        ...
                        NSLog(@"MEdia duration: %lld", duration.value);
                        
                        NSNumber* maxShots = [NSNumber numberWithInt:10];
                        if(duration.value > 60){
                            maxShots = [NSNumber numberWithInt:14];
                        }else if(duration.value > 120){
                            maxShots = [NSNumber numberWithInt:20];
                        }
                        
                        AVAssetImageGenerator *generator = [AVAssetImageGenerator assetImageGeneratorWithAsset:asset];

                        for (int i = 0; i < maxShots.integerValue; i++)
                        {
                            CMTime snapshot = CMTimeMake((duration.value / maxShots.integerValue) * i, duration.timescale);
                            ...
                            mediaData = UIImageJPEGRepresentation(thumbnail, 0.8);
                            
                            NSString* thumbPath = [NSString stringWithFormat:@"%@%s%d%s", imagePath, "-", i, ".jpg"];
                            
                            //writing image to a file
                            NSError* err = nil;
                            if ([mediaData writeToFile:thumbPath
                                    options:NSAtomicWrite
                                    error:&err])
                            {
                                // add thumb to array
                                ...
                                if (err) {
                                    NSLog(@"Error saving video thumbnail: %@", [err localizedDescription]);
                                }
                            }
                        }
                ...
                if (assets && assets.count > 0) {
                    [[PHImageManager defaultManager] requestImageDataForAsset:assets[0]
                        ...
                        resultHandler:^(NSData* _Nullable imageData, NSString* _Nullable dataUTI, UIImageOrientation orientation, NSDictionary* _Nullable info) {
                                if (imageData) {
                                    NSLog(@"FIRST ImageData: %@", imageData);
